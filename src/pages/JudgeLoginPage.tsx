...
export function JudgeLoginPage() {
  ...
  return (
    <div className="relative min-h-screen overflow-hidden bg-slate-50 text-slate-900 dark:bg-slate-950 dark:text-white">
      ...
      <div className="relative mx-auto flex min-h-screen w-full max-w-6xl flex-col gap-6 sm:gap-8 lg:gap-10 px-3 sm:px-4 py-6 sm:py-8 lg:py-10 md:px-6 lg:flex-row lg:items-center lg:gap-12 lg:px-10">
        {/* Hero / Landing */}
        <motion.div
        ...
        >
          <div className="inline-flex items-center gap-1.5 sm:gap-2 rounded-full border border-slate-200 bg-white/80 px-2 sm:px-3 py-0.5 sm:py-1 text-[10px] sm:text-xs font-medium text-slate-600 shadow-sm backdrop-blur dark:border-white/10 dark:bg-white/5 dark:text-slate-200">
            ...
            Live Tabulation for MR & MISS TEEN 2025
          </div>
          ...
          <p className="mt-3 sm:mt-4 max-w-2xl text-sm sm:text-base lg:text-lg text-slate-600 dark:text-slate-300">
          ...
          <div className="mt-4 sm:mt-6 grid grid-cols-2 gap-2 sm:gap-3">
            {[
              ...
              <div
                key={item.label}
                className="flex items-center gap-2 sm:gap-3 rounded-lg sm:rounded-xl border border-slate-200 bg-white/70 px-2 sm:px-4 py-2 sm:py-3 shadow-sm dark:border-white/10 dark:bg-white/5"
              >
                <item.icon className="h-4 w-4 sm:h-5 sm:w-5 text-emerald-500 flex-shrink-0" />
                <p className="text-[10px] sm:text-sm font-medium text-slate-800 dark:text-slate-100">{item.label}</p>
              </div>
            ))}
            </div>

          <div className="mt-6 sm:mt-8 hidden lg:block">
            <p className="text-xs sm:text-sm font-medium uppercase tracking-[0.15em] sm:tracking-[0.2em] text-slate-500 dark:text-slate-400">
              Participating Schools
            </p>
            <div className="mt-2 sm:mt-3 flex flex-wrap items-center gap-2 sm:gap-3 rounded-xl sm:rounded-2xl border border-slate-200 bg-white/70 p-2 sm:p-4 shadow-sm dark:border-white/10 dark:bg-white/5">
              {SCHOOL_LOGOS.map((logo) => (
                ...
                >
                  <img
                    src={logo.src}
                    alt={logo.name}
                    className="h-6 w-6 sm:h-10 sm:w-10 rounded-md sm:rounded-lg object-cover ring-1 ring-slate-200 dark:ring-white/10 flex-shrink-0"
                  />
                  <span className="text-[10px] sm:text-sm font-semibold text-slate-800 dark:text-slate-100 truncate">{logo.name}</span>
              </div>
              ))}
            </div>
          </div>
        ...
        >
          ...
          <Card className="rounded-xl sm:rounded-2xl border border-slate-200/80 bg-white/80 shadow-xl shadow-slate-900/5 backdrop-blur dark:border-white/10 dark:bg-slate-900/70 dark:shadow-black/30">
            <CardHeader className="pb-3 sm:pb-4 text-center px-4 sm:px-6">
              <CardTitle className="text-xl sm:text-2xl lg:text-3xl font-semibold text-slate-900 dark:text-white">Judge Portal</CardTitle>
            ...
            <CardContent className="px-4 sm:px-6">
              <form onSubmit={onSubmit} className="space-y-4 sm:space-y-5">
                <div className="space-y-1.5 sm:space-y-2">
                  <Label htmlFor="username" className="text-xs sm:text-sm font-medium text-slate-700 dark:text-slate-200">
                    Username
                  </Label>
                  <Input
                    id="username"
                  ...
                  />
                  {errors.username && (
                    <p className="text-[10px] sm:text-xs text-rose-600 dark:text-rose-400">{errors.username.message}</p>
                  )}
                </div>
                <div className="space-y-1.5 sm:space-y-2">
                ...
                <div className="flex items-center justify-center gap-2 rounded-lg sm:rounded-xl border border-slate-200/80 bg-slate-50 px-2 sm:px-3 py-1.5 sm:py-2 text-[10px] sm:text-xs text-slate-600 dark:border-white/10 dark:bg-white/5 dark:text-slate-300">
                  ...
                  <span>Your session stays encrypted and tied to your assigned division.</span>
                </div>
                <div className="text-center">
                  <Button
                    type="button"
